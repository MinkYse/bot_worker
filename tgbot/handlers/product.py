...
@product_router.callback_query(StateFilter(None))
async def material_handler(callback: CallbackQuery, state: FSMContext):
    data = str(callback.data).split('-')
    await state.update_data(material=data[1])
    await state.update_data(material_index=int(data[0]))
    """file = FSInputFile(fr'tgbot/media/{size_variants["photos"][0]}')
    await callback.message.answer(messages_from_product['material'])
    await callback.message.answer_photo(file, caption=material_variants['messages'][0],
                                        reply_markup=material_choice('-1', '0', '1'),
                                        parse_mode='html')"""
...
async def discount_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(messages_from_product['discount'], reply_markup=offer_keyboard)
